...
func moduleInputsDiff() (err error) {
	...
	err = readYamlFilesInDir(filepath.Join(pipelineModulesDir, "*", "*"), func(filePath string, fileContent Module) (breakFu bool, err error) {
		...
		inputsIndex.Set(fileContent.GetName(), serviceName, cloudName, fileContent)
		return // true, nil
	})

	// inputsIndexYamlBytes, _ := yaml.Marshal(inputsIndex)
	// fmt.Println(string(inputsIndexYamlBytes))
...
func (ind moduleInputsIndex) Set(moduleName, serviceName, cloudName string, module Module) {
	...
	allInputs := module.ExtractInputs()
